...
def temperature_changes(T, amp, f, t):
    change = 187
    return (amp-np.abs(change*np.sin( np.pi * t*2/100)))*(60*60*24) # HÄR HAR VI ÄNDRAT
        ...
        for col in range(col_indices):
            ...
            for i in range(4):
                ...
                if T[neighbor_row, neighbor_col] <= 273:
                    CPm[i] = Cp_ice * m_ice
                    rad[i] = solarenergy*0.01 #absorption of ice
                else:
...
Max_time = 360
n = 500
amp, f, S, sigma = 204, 10, 1364, 5.67 * 10**(-8) # DET SVÄNGDE GANSKA SNYGGT OM AMP = 104
solarenergy = 273
...
T = initialize_system(N,percent_ice)
